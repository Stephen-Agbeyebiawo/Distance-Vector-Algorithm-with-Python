...
def save_config():
    config['Configuration'] = {'router.name': R.name,'interface1.name' : interface1.name,'interface2.name' : interface2.name,'interface1.cost' : interface1.cost,'interface2.cost' : interface2.cost,'neighbor1.name' : neighbor1.name,'neighbor2.name' : neighbor2.name,'neighbor1.interface' : neighbor1.interface,'neighbor2.interface' : neighbor2.interface,'sleeptime' : sleeptime,'deadtime' : deadtime,'interface1.state' : interface1.state,'interface2.state' : interface2.state}
...
sending = True
listening = True
interface1 = interface(30001,2,"down")
interface2 = interface(30002,3,"down")
...
neighbor1 = neighbor(10,10002,"down")
neighbor2 = neighbor(40,40002,"down")

interfaceList = [interface1,interface2]
interfaceDict = {str(interface1.name):interface1,str(interface2.name):interface2} # Used to get cost of interface to be added to received DV database List
neighborDBList = [neighbor1,neighbor2]
neighborCounterDict = {str(neighbor1.name):neighbor1.counter,str(neighbor2.name):neighbor2.counter} # Used to set the counter of the neighbors
neighborStateDict = {str(neighbor1.name):neighbor1.state,str(neighbor2.name):neighbor2.state}
